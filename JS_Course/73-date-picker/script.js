...
const datePickerGridDateContainer = document.querySelector(".date-picker-grid-dates");
...
function setSelectedDateAsSelected(dates) {
	dates.forEach((date) => {
		...
		if (isSameDay(fromUnixTime(date.dataset.utcdate), currentSelectedDate)) date.classList.add("selected");
	});
}

function showWeeksProportionalToMonthLength(entireMonthByDays, firstWeekStart, dates) {
	// Add another row to the calendar, if the month is spread over 6 weeks
	const thursday = 4;
	const friday = 5;
	if (
		(entireMonthByDays.length > 30 && firstWeekStart > thursday) ||
		(entireMonthByDays.length > 28 && firstWeekStart > friday)
	) {
		for (let i = 35; i < 42; i++) {
...
function setBeforeMonthDays(selectedDate, dates, firstWeekStart) {
	...
	for (let weekDay = 0; weekDay < 7 - (7 - firstWeekStart); weekDay++) {
		...
		date.textContent = `${format(subDays(lastDayOfLastMonth, 6 - (7 - firstWeekStart) - weekDay), "d")}`;
		date.dataset.utcdate = getUnixTime(subDays(lastDayOfLastMonth, 6 - (7 - firstWeekStart) - weekDay));
	}
}

function setRegularDays(dates, firstWeekStart, entireMonthByDays, selectedDate) {
	for (let week = 0; week < 34; week += 7) {
		for (let weekDay = 0; weekDay <= 7 && week + weekDay < entireMonthByDays.length; weekDay++) {
			dates[weekDay + firstWeekStart + week].textContent = `${week + weekDay + 1}`;
...
function setAfterMonthDays(dates, entireMonthByDays, weeksInMonthInDays) {
	for (let weekDay = getDay(entireMonthByDays[entireMonthByDays.length - 1]) + 1, i = 1; weekDay < 7; weekDay++, i++) {
		const day = dates[weekDay + weeksInMonthInDays];
		day.classList.add("date-picker-other-month-date");
		day.textContent = i;
		day.dataset.utcdate = getUnixTime(addDays(startOfMonth(addMonths(currentDate, 1)), i - 1));
	}
}
...
function setupDates(selectedDate) {
	...
	let weeksInMonthInDays = showWeeksProportionalToMonthLength(entireMonthByDays, firstWeekStart, dates);
	// console.log(entireMonthByDays);

	setBeforeMonthDays(selectedDate, dates, firstWeekStart);

	setRegularDays(dates, firstWeekStart, entireMonthByDays, selectedDate);

	setAfterMonthDays(dates, entireMonthByDays, weeksInMonthInDays);
...
datePickerGridDateContainer.addEventListener("click", (e) => {
	...
	if (currentSelectedBtn != null) currentSelectedBtn.classList.remove("selected");
